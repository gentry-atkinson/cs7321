...
namespace GazeTrackingLibrary.Utils
{
    ...
    public class Operations
    {
		...
		static public int Min(double[] num)
		{
			int indexMin = 0;
		...
		static public int Max(double[] num)
		{
			int indexMax = 0;
			double max = 0;
			for (int i = 0; i < num.Length; i++)
		...
		/// <param name="points">Array of 4 points</param>
		/// <returns>Ordered array of the 4 points</returns>
		static public Point[] OrderPoints(Point[] points)
		{
			int N = 4;
		...
		public static Matrix<double> SolveLeastSquares(Matrix<double> A, Matrix<double> b)
        {
            ...
            Matrix<double> x = new Matrix<double>(N, 1);

            Matrix<double> U = new Matrix<double>(M, M);
            Matrix<double> F = new Matrix<double>(M, N);
            Matrix<double> V = new Matrix<double>(N, N);

            Matrix<double> c = new Matrix<double>(M, 1);
            Matrix<double> y = new Matrix<double>(N, 1);
		...
		static public Matrix<double> Homography2D(Matrix<double> m1, Matrix<double> m2)
		{
			//int N = m1.Cols;
			...
			CvInvoke.cvFindHomography(m1.Ptr, m2.Ptr, H.Ptr, HOMOGRAPHY_METHOD.DEFAULT, 0, new System.IntPtr());
