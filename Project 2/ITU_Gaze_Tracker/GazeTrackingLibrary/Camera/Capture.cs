...
namespace GazeTrackingLibrary.Camera
{
    ...
    /// </summary>
        ...
        public Capture(DsDevice device, int frameRate)
        {
            Initialize();
        ...
        public Capture(DsDevice device, int frameRate, int width, int height)
        {
            Initialize();
        ...
        ~Capture()
        {
            ...
            GTSettings.Current.Camera.OnCameraControlPropertyChanged -= OnCameraControlPropertyChanged;
            GTSettings.Current.Camera.OnVideoProcAmpPropertyChanged -= OnVideoProcAmpPropertyChanged;
            GTSettings.Current.Camera.OnVideoControlFlagsChanged -= OnVideoControlFlagsChanged;
        }
        ...
        public void Start()
        {
            if (isRunning || mediaControl == null) return;
        ...
        private bool SetupGraph(DsDevice dev, int frameRate, int width, int height)
        {
            ...
            try
            {
                ...
                */
                ...
                Devices.Current.Cameras[GTSettings.Current.Camera.DeviceNumber].DirectshowDevice.Mon.BindToObject(
                    null,
                    null,
                    ref iid_IBaseFilter,
                    out obj);
            ...
            foreach (VideoProcAmpProperty item in Enum.GetValues(typeof (VideoProcAmpProperty)))
            {
                switch (item)
                {
                    case VideoProcAmpProperty.Brightness:
                        ...
                        GTSettings.Current.Camera.BrightnessMinimum = min;
                        GTSettings.Current.Camera.BrightnessMaximum = max;
                        GTSettings.Current.Camera.BrightnessStepping = steppingDelta;
                        GTSettings.Current.Camera.BrightnessDefault = defaultValue;
                        break;
                    ...
                    case VideoProcAmpProperty.Saturation:
                        videoProcAmp.GetRange(VideoProcAmpProperty.Saturation, out min, out max, out steppingDelta,
                                              out defaultValue, out flags);
                        GTSettings.Current.Camera.SaturationMinimum = min;
                        GTSettings.Current.Camera.SaturationMaximum = max;
                        GTSettings.Current.Camera.SaturationStepping = steppingDelta;
                        GTSettings.Current.Camera.SaturationDefault = defaultValue;
                        break;
                    ...
                    case VideoProcAmpProperty.Sharpness:
                        videoProcAmp.GetRange(VideoProcAmpProperty.Sharpness, out min, out max, out steppingDelta,
                                              out defaultValue, out flags);
                        GTSettings.Current.Camera.SharpnessMinimum = min;
                        GTSettings.Current.Camera.SharpnessMaximum = max;
                        GTSettings.Current.Camera.SharpnessStepping = steppingDelta;
                        GTSettings.Current.Camera.SharpnessDefault = defaultValue;
                        break;

                    case VideoProcAmpProperty.Contrast:
                        videoProcAmp.GetRange(VideoProcAmpProperty.Contrast, out min, out max, out steppingDelta,
                                              out defaultValue, out flags);
                        GTSettings.Current.Camera.ContrastMinimum = min;
                        GTSettings.Current.Camera.ContrastMaximum = max;
                        GTSettings.Current.Camera.ContrastStepping = steppingDelta;
                        GTSettings.Current.Camera.ContrastDefault = defaultValue;
                        break;
                ...
                switch (item)
                {
                    case CameraControlProperty.Exposure:
                        cameraControl.GetRange(CameraControlProperty.Exposure, out min, out max, out steppingDelta,
                                               out defaultValue, out flags);
                        GTSettings.Current.Camera.ExposureMinimum = min;
                        GTSettings.Current.Camera.ExposureMaximum = max;
                        GTSettings.Current.Camera.ExposureStepping = steppingDelta;
                        GTSettings.Current.Camera.ExposureDefault = defaultValue;
                        break;
                    case CameraControlProperty.Focus:
                        cameraControl.GetRange(CameraControlProperty.Focus, out min, out max, out steppingDelta,
                                               out defaultValue, out flags);
                        GTSettings.Current.Camera.FocusMinimum = min;
                        GTSettings.Current.Camera.FocusMaximum = max;
                        GTSettings.Current.Camera.FocusStepping = steppingDelta;
                        GTSettings.Current.Camera.FocusDefault = defaultValue;
                        break;
                    case CameraControlProperty.Zoom:
                        cameraControl.GetRange(CameraControlProperty.Focus, out min, out max, out steppingDelta,
                                               out defaultValue, out flags);
                        GTSettings.Current.Camera.ZoomMinimum = min;
                        GTSettings.Current.Camera.ZoomMaximum = max;
                        GTSettings.Current.Camera.ZoomStepping = steppingDelta;
                        GTSettings.Current.Camera.ZoomDefault = defaultValue;
                        break;
