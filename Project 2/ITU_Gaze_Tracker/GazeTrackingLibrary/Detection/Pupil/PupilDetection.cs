...
namespace GazeTrackingLibrary.Detection.Pupil
{
    public class PupilDetection 
    {
        ...
        public bool DetectPupil(Image<Gray, byte> inputImage, TrackData trackData)
        {
            ...
            if (eye == EyeEnum.Left)
                PupilGrayLevel = GTSettings.Current.Processing.PupilThresholdLeft;
            else
                PupilGrayLevel = GTSettings.Current.Processing.PupilThresholdRight;

            MinPupilSize = GTSettings.Current.Processing.PupilSizeMinimum;
            MaxPupilSize = GTSettings.Current.Processing.PupilSizeMaximum;

            int min = (int)Math.Round(Math.PI*Math.Pow(MinPupilSize, 2));
            int max = (int)Math.Round(Math.PI*Math.Pow(MaxPupilSize, 2));
        ...
        //if(eye == EyeEnum.Left)
        //    this.pupilGrayLevel = GTSettings.Current.Processing.PupilThresholdLeft;
        //else
        //    this.pupilGrayLevel = GTSettings.Current.Processing.PupilThresholdRight;
        ...
        public bool DetectTwoPupils(Image<Gray, byte> inputImage, TrackData trackData)
        {
            ...
            PupilGrayLevel = GTSettings.Current.Processing.PupilThreshold;
            MinPupilSize = GTSettings.Current.Processing.PupilSizeMinimum;
            MaxPupilSize = GTSettings.Current.Processing.PupilSizeMaximum;
