...
namespace GazeTracker.Calibration
{
    public partial class SharingControlUC : UserControl
    {
        ...
        public void SendData(Tracker tracker, BitmapSource bitmapImage)
        {
            ...
            sb.AppendLine(CameraControl.Instance.DeviceName);
            sb.AppendLine(CameraControl.Instance.Width + "\t" + CameraControl.Instance.Height + "\t" +
                          CameraControl.Instance.FPS + "\t");
            sb.AppendLine(GTSettings.Current.Processing.TrackingMethod + "\t" + tracker.FPSTracking);
        ...
        private void SendDataGazeGroup(string calibrationData, BitmapSource bitmapImage)
        {
            ...
            // Check dir
            if (Directory.Exists(dirUri.LocalPath) == false)
                Directory.CreateDirectory(dirUri.LocalPath);

            // Save to files
            var fileCalibrationData = new FileInfo(dirUri.LocalPath + "\\" + id + ".txt");
            SaveCalibrationData(fileCalibrationData, calibrationData);

            var fileImage = new FileInfo(dirUri.LocalPath + "\\" + id + ".jpg");
