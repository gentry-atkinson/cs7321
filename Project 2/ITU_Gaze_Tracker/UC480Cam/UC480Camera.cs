    ...
    public class UC480Camera
    {
       ...
       public const string DRIVER_DLL_NAME = "uc480_64.dll";
        ...
        public const int IS_CM_NOT_SUPP_IN_CURR_DISPLAYMODE = 182;
                         // color mode is not supported in the current display mode
        ...
        public const uint FDT_CMD_GET_SEARCH_ANGLE_ENABLE = 7;
                          /* Get the current setting of search angle enable.              */
        ...
        public struct UC480_ETH_DEVICE_INFO_HEARTBEAT
        {
            ...
            public UC480_ETH_IP_CONFIGURATION ipcfgCurrentIpCfg; // current IP configuration
        ...
        private static extern int is_GetColorConverter(int hCam, int ColorMode, ref int pCurrentConvertMode,
        ...
        public int GetEthDeviceInfo(int iDeviceID, ref UC480_ETH_DEVICE_INFO pDeviceInfo)
        {
            ...
            if (ret == IS_SUCCESS)
            {
                ...
                off += 4;
                off += 4;
                pDeviceInfo.infoDevHeartbeat.ipcfgCurrentIpCfg.ipAddress.dwAddr =
                    (uint) ((pTemp[off + 3] << 24) + (pTemp[off + 2] << 16) + (pTemp[off + 1] << 8) + (pTemp[off]));
                off += 4;
                pDeviceInfo.infoDevHeartbeat.ipcfgCurrentIpCfg.ipSubnetmask.dwAddr =
                    (uint) ((pTemp[off + 3] << 24) + (pTemp[off + 2] << 16) + (pTemp[off + 1] << 8) + (pTemp[off]));
                off += 4;
                off += 4;
        ...
        public int GetColorConverter(int ColorMode, ref int pCurrentConvertMode, ref int pDefaultConvertMode,
                                     ref int pSupportedConvertModes)
        {
            if (m_hCam == 0)
                return IS_INVALID_CAMERA_HANDLE;

            return is_GetColorConverter(m_hCam, ColorMode, ref pCurrentConvertMode, ref pDefaultConvertMode,
