namespace GazeTrackerClient
{
    public class Commands
    {
        ...
        public static bool CommandHasParameter(string cmd)
        {
            if (cmd.Equals(CalibrationParameters))
                return true;
            else if (cmd.Equals(StreamFormat))
                return true;
            else if (cmd.Equals(EyeTrackerParameters))
                return true;
            else if (cmd.Equals(LogPathSet))
                return true;
            else if (cmd.Equals(LogWriteLine))
                return true;
            else if (cmd.Equals(CameraParameters))
                return true;
            else
                return false;
        }
...
// Not implemented yet.. no MIME eye image is sent..
//private class BitmapCreator
//{
...
//if (OnEyeImageCapture != null)
//    OnEyeImageCapture(bmpCreator.CreateBitmapFromMime(Int32.Parse(cmd[1]), Int32.Parse(cmd[2]), cmd[3]));

//public Image CreateBitmapFromMime(int width, int height, string data)
//{
//    // Not complete..
//    //System.Windows.Forms.MessageBox.Show("mime-lenght: " + data.Length.ToString());
//    System.IO.MemoryStream ms = new System.IO.MemoryStream(DecodeToBytes(data));
//    Image returnImage = Image.FromStream(ms);
//    return returnImage;
//}

//    private byte[] DecodeToBytes(string s)
//    {
//        if (s == null)
